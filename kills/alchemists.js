...
module.exports = async client => {
  ...
  alivePlayers.forEach(async player => {
    ...
    if (Array.isArray(guy.poisoned)) { // checks if they property is an array
      
      // check if at least one of their attackers is alive
      if (guy.poisoned.some(p => alivePlayers.includes(p))) { // checks if at least 1 alchemist who poisoned them is alive
        ...
        dayChat.send(`${getEmoji("blackp", client)} The Alchemist killed **${guy.username} ${players.indexOf(guy.id)} (${getEmoji(guy.role.toLowerCase().replace(/\s/g, "_"), client)} ${guy.role})**!`) // send the message
