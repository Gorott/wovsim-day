...
module.exports = async client => {
  ...
  for (bomber in bombers) {
    let attacker = db.get(`player_${bomber}`)
    
    if (attacker.status === "Alive") {
      
      if (attacker.target) {
        
        if (attacker.setBombs == true) {
          ...
          attacker.target.forEach(async target => {
            ...
            if (guy.status === "Alive") {
              let attackedPlayerRoles = attackedPlayer.roles.cache.map(r => r.name === "Alive" ? "892046207428476989": r.id)
              dayChat.send(`${getEmoji("explode", client)}  **${players.indexOf(guy.id)+1} ${guy.username} (${getEmoji(guy.role?.toLowerCase()?.replace(/\s/g, "_"), client)} ${guy.role})* was killed by an explosion!`)
