...
module.exports = async client => {
  ...
  alivePlayers.forEach(player => {
    ...
    if (db.get(`player_${player}`).vote) votes[player] = player.vote // add the vote to the votes object
  ...
  Object.entries(votes).forEach(vote => {
    ...
    if (player.hasBread === true) countedVotes[vote[1]] += 1 // add another vote if they have a bread (given by Baker)
  ...
  if (sortedVotes[0][1] < Math.floor(alivePlayers.length / 2) || (sortedVotes.length > 1 && sortedVotes[0][1] === sortedVotes[1][1])) {
    ...
    if (["Handsome Prince", "Idiot"].includes(guy.role)) {
      ...
      if (guy.role === "Idiot") { 
        await dayChat.send(`${getEmoji("votingme", client)} The Villagers tried to lynch **${players.indexOf(guy.id)+1} ${guy.username} (${getEmoji("handsome_prince", client)} Idiot), but instead made them lose their ability to vote.`) 
